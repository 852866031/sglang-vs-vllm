#!/usr/bin/env python3
"""
run_benchmark.py — mini-sglang timeline benchmark (base model only).

Mirrors the other engines' benchmarks, with two key differences:

  - mini-sglang has NO LoRA support, so requests do not specify an adapter.
    All requests run against the base Llama-3.1-8B. (The round-robin LoRA
    selection used for vLLM/SGLang doesn't apply here.)
  - mini-sglang's /generate streams raw text deltas (not JSON), terminated
    by `data: [DONE]`. There is no /health endpoint, so readiness is
    probed via /v1/models.

Output CSV columns match example_script/example_output.csv exactly:
...
# ----------------------------
# Readiness probe (mini-sglang has no /health endpoint)
# ----------------------------
async def try_ready(session: aiohttp.ClientSession, server: str, timeout_s: float = 1.5) -> bool:
    base = server.rstrip("/")
    try:
        async with session.get(f"{base}/v1/models", timeout=timeout_s) as resp:
...
async def wait_for_server(server: str, max_wait_s: float = 900.0, poll_period_s: float = 1.0) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            if await try_ready(session, server):
                _emit(f"[orchestrator] Server is up ✅ at {server}")
                return
            if time.monotonic() - t0 > max_wait_s:
                raise TimeoutError(f"Server didn't become ready within {max_wait_s:.1f}s")
            await asyncio.sleep(poll_period_s)


# ----------------------------
# Request: mini-sglang native /generate
#
# Stream format (from minisgl/server/api_server.py::stream_generate):
#   data: <incremental_text>\n     # one line per token
#   data: [DONE]\n
# Each `data:` line carries the delta token text directly (NOT JSON).
# ----------------------------
async def send_one_streaming(
    ...
    try:
        ...
        return (idx, t_rel, latency, "ok", ttft, avg_tbt, worst_tbt,
                tokens_received, gen_chars, saw_done, None)
    except Exception as e:
        latency = time.monotonic() - t_send
        return (idx, t_rel, latency, f"error:{type(e).__name__}",
                None, None, None, tokens_received, gen_chars, saw_done, None)
...
async def main() -> None:
    ...
    ap.add_argument("--out_csv", default=str(DEFAULT_OUTPUT_DIR / "mini_sglang_results.csv"))
    ap.add_argument("--launcher", default=str(SCRIPT_DIR / "launch_mini_sglang.py"))
    ...
    ap.add_argument("--launcher_args", default="",
                    help="Extra args passed verbatim to launch_mini_sglang.py.")
    args = ap.parse_args()
    ...
    try:
        if not args.no_spawn:
            ...
            async def pump() -> None:
                """Forward server output, splitting on both '\\n' and '\\r' so
                server-side tqdm progress bars don't appear to hang."""
