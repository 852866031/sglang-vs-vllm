#!/usr/bin/env python3
"""
run_benchmark.py — vLLM multi-LoRA timeline benchmark.

Mirrors example_script/auto_benchmark.py:
  - Spawns the vLLM OpenAI-compatible server (launch_vllm.py) in its own
    process group and streams server logs.
  - Waits for the server to become healthy.
  - Warmup phase: replays the first N timeline rows on a normalized schedule
    (warmup results are NOT written to the output CSV).
  - Main phase: replays the full timeline at the timestamps in timeline.csv.
  - Writes per-request metrics to outputs/<run_name>.csv with columns
    matching example_script/example_output.csv:
      idx, t_rel_s, latency_s, status, ttft_s, avg_tbt_s, worst_tbt_s

LoRA selection is round-robin per row_id: even rows -> llama3-toy-lora-0,
odd rows -> llama3-toy-lora-1. Requests stream so we can measure TTFT and
per-token gaps (TBT).
"""
import argparse
import asyncio
...
def make_prompt_from_length(prompt_length: int) -> str:
    """Same construction as example_script/auto_benchmark.py."""
    base = "Instruction:\n"
    tail = "\n### Response: "
...
async def wait_for_server(server: str, max_wait_s: float = 600.0, poll_period_s: float = 1.0) -> None:
    t0 = time.monotonic()
...
# ----------------------------
# Request: vLLM OpenAI-compatible /v1/completions with streaming
# ----------------------------
async def send_one_streaming(
    ...
    """
    POST /v1/completions with stream=True. Measures:
      latency      — total wall-clock for the full response
      ttft         — time from POST to first non-empty token chunk
      avg_tbt      — mean inter-token gap (between consecutive non-empty chunks)
      worst_tbt    — max inter-token gap

    The LoRA adapter name is passed as the `model` field (vLLM's convention
    when --lora-modules name=path is used).
    """
    ...
    payload = {
        ...
        "temperature": 0.0,
        "stream": True,
        # ignore_eos forces the model to produce exactly max_tokens, matching
        # the reference benchmark's `ignore_eos=True` behavior. Otherwise the
        # toy adapters may emit EOS very early and skew TBT stats.
        "ignore_eos": True,
    ...
    tbts: List[float] = []
    tokens_received = 0          # count of non-empty SSE text chunks
    gen_chars = 0                # cumulative length of generated text
    saw_done = False             # confirmed server-side stream terminator
    ...
    try:
        async with session.post(url, json=payload, timeout=timeout) as resp:
            ...
            async for raw in resp.content:
                ...
                if not choices:
                    continue
                # Capture finish_reason from whichever chunk carries it.
        ...
        return (idx, t_rel, latency, f"error:{type(e).__name__}",
                None, None, None, tokens_received, gen_chars, saw_done, finish_reason)
...
async def run_timeline(
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        async def _run(row: TimelineRow) -> None:
            ...
            # Splice in the scheduled-time column so the plot can show requests
            # on the timeline they were SUPPOSED to fire at, not when the
            # orchestrator actually dispatched them (which drifts under load).
...
def _summarize_token_counts(results, timeline_rows) -> None:
    """Print min/median/max of tokens_received and what fraction of requests
    received exactly the requested max_new_tokens. Cheap sanity check that
    the engine actually generated full responses — guards against silent
    truncation making latency numbers look better than they should."""
...
async def main() -> None:
    ...
    ap.add_argument("--out_csv", default=str(DEFAULT_OUTPUT_DIR / "vllm_results.csv"))
    ...
    ap.add_argument("--no-spawn", action="store_true",
                    help="Skip launching the server; assume one is already "
                         "running at --host:--port (useful for debugging).")
    ap.add_argument("--launcher_args", default="",
                    help="Extra args passed verbatim to launch_vllm.py.")
    args = ap.parse_args()
    ...
    try:
        if not args.no_spawn:
            ...
            async def pump() -> None:
                ...
                while True:
                    ...
                    if not raw:
                        return
                    _emit("[server] " + raw.decode("utf-8", errors="replace").rstrip())
        ...
        # Warmup selection: min of count cap and duration cap.
